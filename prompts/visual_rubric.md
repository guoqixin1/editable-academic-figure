...
19. **行级空白率**：任何水平/垂直条带区，内容占比 <50% → 标记「过空」，压缩条带高度或合并相邻空带。
20. **标题行平衡**：画布顶部题字行不允许只有标题 + 模块阴影顶（要么删去在图标题、要么让首行有实质内容模块）。
21. **箭头-背景对比**：箭头颜色与所压底色的亮度差 ≥60（0–255 通道均值差）；路径上逐段检查，浅底勿用浅灰箭头。
22. **统计图 sketch 可辨识度**：条形/柱状 sketch 相邻高度差 ≥15%，否则视为「无信息装饰」——换数据或删 sketch。
23. **卡片内边距**：卡内容（文字行高之和）占卡高 ≥55%；过空则减 `h` / 加 body / 收 `box_pad_y`。
...
机检抓客观硬伤，目检抓「像不像论文图」。新增 D 节是在抓五种常见糊弄：条带太空、顶行只挂个标题、箭头融进背景、假柱状图、卡片里字太少。逐格放大看完再交付。
