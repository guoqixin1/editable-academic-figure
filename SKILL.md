...
description: >-
  ...
  emits a geometry lint report. Use this skill when the user asks to draw or
  edit a scientific figure, reproduce a paper figure, replace a placeholder
  experiment image with a real one, tweak an existing figure.yaml, or mentions
  "scifig", "figure.yaml", "科研作图", "论文架构图", or "复现论文图". Never
  generate real experimental plots (spectra, waveforms, heatmaps, quantitative
  curves) — those must always use placeholder assets that the user fills in.
---
...
- **从零作图**：走 [prompts/AGENT_WORKFLOW.md](prompts/AGENT_WORKFLOW.md) 的分阶段流程（需求拆解 → 占位布局 → 抽卡 → 评审 → 交付）。
- **复现论文图**：参照 `examples/rep_*/`（`rep_evdispatch`, `rep_uavedge`, `rep_codriving`, `rep_d3pgmodel`, `rep_ensemble`），照抄写法。

## 视觉评审

机检（lint）过关后按 [prompts/visual_rubric.md](prompts/visual_rubric.md) 做目检：对齐、留白、箭头语义、配色、素材融入、上下标、marker/tokens 是否表意。
